...
; (function () {
	...
	*/
	...
	class Controller {
		...
		showIcons(opponent, [x, y], iconClass) {
			...
			function fn() {
				...
				if (iconClass == 'red-cross') {
					span.innerHTML = '<img src="http://pngimg.com/uploads/explosion/explosion_PNG15355.png" style="width:35px; height:35px">';
				}
				if (iconClass === 'dot') {
					span.innerHTML = '<img src="./img/miss.png" style="width:35px; height:35px">';
				}
		...
		makeShot(e) {
			...
			switch (v) {
				...
				case 4:
					Controller.showServiceText('За цими координатами ви вже стріляли!');
					break;
			}
		}
		...
		miss(x, y) {
			...
			if (this.player === human) {
				text = "Ви промахнулися. Стріляє комп'ютер.";
				this.player = computer;
				this.opponent = human;
				compShot = true;
				setTimeout(() => this.makeShot(), 2000);
			} else {
				text = "Комп'ютер промахнувся. Ваш постріл.";
		...
		hit(x, y) {
			...
			text = (this.player === human) ? 'Вітаємо! Ви  влучили. Ваш постріл.' : "Комп'ютер потрапив в ваш корабель. постріл компютера";
			setTimeout(() => Controller.showServiceText(text), 400);
			if (this.player === human) {
				userHits++;
				document.getElementById('userHits').innerHTML = `
				Ви влучили ${userHits} з 20
				`
			} else {
				computerHits++;
				document.getElementById('computerHits').innerHTML = `
				Комп'ютер влучив ${computerHits} з 20
				`
			}
	...
	buttonPlay.addEventListener('click', function (e) {
		...
		toptext.innerHTML = 'Морський бій між ескадрами';
